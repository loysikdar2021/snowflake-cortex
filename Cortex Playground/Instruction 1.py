This playbook provides a structured, step-by-step guide to experimenting with Snowflake Cortex using the built-in `SNOWFLAKE_SAMPLE_DATA`. We will cover Generative AI (LLMs), Machine Learning (Forecasting), and Retrieval Augmented Generation (RAG).

**Prerequisites:**

  * A Snowflake account (Cortex is available in specific AWS and Azure regions).
  * `ACCOUNTADMIN` or `SYSADMIN` role (or a role with privileges to create databases and warehouses).
  * An active Virtual Warehouse (e.g., `COMPUTE_WH`).
...
**1. Create the Playground Environment**
Run the following SQL to set up your workspace:
...
Let's determine if the comments left on orders are positive or negative.
...
Now, let's pick a long comment, summarize it, and translate it to French.
...
We need to aggregate the sample data into a time-series format (Date + Value).
...
Snowflake manages the underlying ML infrastructure automatically.
...
Ask the model to predict the next 10 days.
...
We will select a subset of order comments and generate embeddings for them using `EMBED_TEXT_768` (Snowflake's arctic-embed model).
...
FROM SNOWFLAKE_SAMPLE_DATA.TPCH_SF1.ORDERS
LIMIT 1000; -- Limiting for speed in this demo
```

**2. Perform Vector Search (The "Retrieval")**
Let's find the most relevant order comment for a specific query: *"Customer is angry about broken packaging."*
...
**3. The "Generation" (Connecting Retrieval to LLM)**
This is the full RAG step. We take the top result from the search above and feed it to the LLM (`mistral-large` or `llama3`) to generate a human-readable response.
...
1.  **Environment:** A dedicated lab for AI experiments.
2.  **Cortex AI:** Applied sentiment analysis and translation on raw text.
3.  **Cortex ML:** Built a time-series forecasting model on shipping data.
4.  **RAG:** Created a vector database from scratch and implemented a semantic search-to-generation pipeline.

**Next Step:**
Would you like to see how to automate this RAG pipeline using **Snowflake Tasks and Streams** so that new order comments are automatically embedded and searchable as they arrive?